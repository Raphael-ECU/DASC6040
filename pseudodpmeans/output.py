...
from pseudodpmeans.clustering.clusteringcomponents import ClusteringModel
...
def _write_cluster_samples(
    ...
    with open(write_name, "w", encoding="utf_8") as output:
...
def _get_output_basename(output_dir, clustering_run_id):
    return os.path.join(output_dir, f"dpmeans_{clustering_run_id}")
